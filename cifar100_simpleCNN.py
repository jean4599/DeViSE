...
def cnn_model(features, labels, train_mode):
    ...
    # simple model
    # conv1
    conv1 = Conv2d('conv1', input_layer, 5, 3, 64, 1, 'SAME', tf.nn.relu)
    print("conv1 tensor: ", conv1)
    # pool1
    pool1 = tf.nn.max_pool(conv1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool1')
    print("pool1 tensor: ", pool1)
    # norm1 (local_response_normalization)
    norm1 = tf.nn.lrn(pool1, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75, name='norm1')
    print("norm1 tensor: ", norm1)
    # conv2
    conv2 = Conv2d('conv2', norm1, 5, 64, 64, 1, 'SAME', tf.nn.relu)
    ...
    # Logits Layer
    logits = tf.layers.dense(inputs=dropout1, units=100, name='logits')
...
def train(X_train, y_train, X_validate, y_validate, train_op, epoch_bound, stop_threshold, batch_size, testing=False):
    ...
    for epoch in range(epoch_bound):
        ...
        if cur_loss < winner_loss:
            ...
            if testing == True:
                save_path = saver.save(sess, "./saved_model/cifar100_simpleCNN/cifar-100_simpleCNN.ckpt")
        else:
            early_stop += 1
...
writer = tf.summary.FileWriter("logs/cifar100_simpleCNN", sess.graph)
...
if TAKE_CROSS_VALIDATION == True:
    for fold in range(1, CROSS_VALIDATION+1):
        ...
        saver.restore(sess, "./saved_model/cifar100_simpleCNN/cifar-100_simpleCNN.ckpt")
